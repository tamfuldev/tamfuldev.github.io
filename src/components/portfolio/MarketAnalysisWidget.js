...
const stockLikeSymbols = new Set(["AAPL", "AMZN", "GOOGL", "MSFT", "NVDA", "TSLA", "VIC", "VHM", "VNM"]);

const normalizeSymbol = (value) => value.trim().replace(/\s+/g, "").toUpperCase();

const getAssetType = (symbol) => {
    if (coinAliases[symbol] || symbol.includes("USDT") || symbol.includes("USDC")) {
        return "CRYPTO";
    }

    return stockLikeSymbols.has(symbol) || /^[A-Z]{3,5}$/.test(symbol) ? "STOCK" : "CRYPTO";
};
...
const getRecommendation = ({ change24h, rsi, trend }) => {
    if (trend.label === "Tăng" && change24h > 0 && rsi < 70) {
        return {
            className: "is-bull",
            label: "MUA",
            reason: "Trend đang cao hơn MA7, RSI chưa quá nóng và momentum 24h vẫn tích cực.",
        };
    }

    if (trend.label === "Giảm" && change24h < 0) {
        return {
            className: "is-bear",
            label: "BÁN",
...
const resolveCoinId = async (symbol) => {
    ...
    const response = await fetch(`${COINGECKO_API_BASE}/search?query=${encodeURIComponent(symbol)}`);
    const data = await response.json();

    if (!response.ok) {
        throw new Error(data?.error || "Không thể tìm coin trên CoinGecko.");
    }
...
const fetchCoinMarket = async (coinId) => {
    ...
    if (!response.ok) {
        throw new Error(data?.error || "Không thể lấy market data từ CoinGecko.");
    }
...
const buildCopyText = (market, analysis) => [
    ...
    `Giá: ${formatCurrency(analysis.currentPrice)} | 24h: ${formatPercent(market.price_change_percentage_24h)} | 7d: ${formatPercent(market.price_change_percentage_7d_in_currency)}`,
    `Trend: ${analysis.trend.label} | RSI: ${analysis.rsi.toFixed(1)} (${analysis.rsiSignal.label})`,
    `Support: ${formatCurrency(analysis.support)} | Resistance: ${formatCurrency(analysis.resistance)}`,
...
const MarketAnalysisWidget = ({ defaultSymbol = "BTC", language = "en" }) => {
    ...
    const handleAnalyze = async (event) => {
        ...
        if (assetType === "STOCK") {
            setAnalysis(null);
            setMarket(null);
            setError("Hiện widget chỉ hỗ trợ crypto qua CoinGecko free API. Chứng khoán VN có thể thêm sau bằng VNSTOCK API.");
            return;
        }
        ...
        try {
            const coinId = await resolveCoinId(normalizedSymbol.replace(/[-_/]?(USDT|USD|USDC)$/i, ""));
        ...
        <div className="portfolio-page">
            <section className="portfolio-market-wrap">
                <div className="portfolio-market-hero">
                    <div>
                        <div className="portfolio-section-label">{"// coingecko_market_analysis"}</div>
                            ...
                            <div className="portfolio-market-analysis">
                                ...
                                <div className="portfolio-market-metrics">
                                    <div>
                                        ...
                                        <strong>{formatCurrency(analysis.totalVolume)}</strong>
                                    </div>
                                    <div>
                                        <span>Market Cap</span>
                                        <strong>{formatCurrency(analysis.marketCap)}</strong>
                                    </div>
                                    <div>
                                        <span>MA7</span>
                                        <strong>{formatCurrency(analysis.ma7)}</strong>
                                    </div>
                                </div>
                                ...
                                <div className="portfolio-market-levels">
                                    <div>
                                        ...
                                        <strong>{formatCurrency(analysis.support)}</strong>
                                    </div>
                                    <div>
                                        <span>Resistance 7d</span>
                                        <strong>{formatCurrency(analysis.resistance)}</strong>
                                    </div>
                                </div>
                                ...
                                <p className="portfolio-market-note">
                                    { language == "vi" ? "Cập nhật" : "Update" }: {analysis.updatedAt ? new Date(analysis.updatedAt).toLocaleString("vi-VN") : "-"}
                                </p>
                            </div>
                        )}
